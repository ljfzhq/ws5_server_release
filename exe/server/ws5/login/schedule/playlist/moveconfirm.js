...
/*
			...
			srcObj.rootobjid = '50ee276ec55e788809000001';
			...
			srcObj.rootobjid = '50ee2e7a7d5ac3b010000001';
			srcArray.push(srcObj);
			...
			srcObj.rootobjid = '50ee2e676158c17016000001';
			srcArray.push(srcObj);
			...
			srcObj.rootobjid = '50ee2e4e06733f3c0a000001';
			srcArray.push(srcObj);
...
*/			
		...
		console.log(targetConflictPlaylistArray);
												...
												//	2.2 if the conflict array include the playlist which is in source playlist array, remove it from conflict array
												rangeSize = dataObj.src.length;
												conflictArraySize = targetConflictPlaylistArray.length;
												for(index = 0; index < conflictArraySize; index ++) {
												   	targetConflictPlaylistArray[index].inSource = false;
													for(i = 0; i < rangeSize; i++) {
														if((targetConflictPlaylistArray[index].start === dataObj.src[i].start) && 
														   (targetConflictPlaylistArray[index].end === dataObj.src[i].end) && 
														   (targetConflictPlaylistArray[index].playlistpath === dataObj.src[i].playlistpath)) {
		...
		console.log(dataObj.src);
														schedule.deletePlaylistArray(dataObj.src, function(err) { 
															if(err) {
																retVal.id = err;
																retVal.msg = helper.retval[err];
