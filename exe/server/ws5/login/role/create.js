...
var CreateRole = function() {
	...
	this.do = function(req, res) {
		...
		logger.debug('enter role/create.js.');
			...
			accountrole.getAccountByID(userid, function(err, accountInfo) {
				...
				privilege.checkPrivilege('system', '/account', 'account', accountInfo.name, 2, function(err, have) {
					if(err) {
						retVal.id = err;
						retVal.msg = helper.retval[err];
						logger.error('error occur when get privilege data from database or has not enough privilege to create role.');
						logger.error(dataObj);
						return res.send(retVal);
					}
					else {
						if(have) {
							//create record in DB
							accountrole.createRole(dataObj, function(err) {
								if(err) {
									retVal.id = err;
									retVal.msg = helper.retval[err];
									logger.error('error occurs when call accountrole.createRole(). ' + err);
									return res.send(retVal);
								}
								else {
									accountrole.getRoleByName(dataObj.name, function(err, obj) {
										retVal.id = err;
										retVal.msg = helper.retval[err];
										if(!err) {
											retVal.status = true;
										...
										logger.debug('return from role/create.js.');
							...
							logger.error('you have not enough privilege to create role.');
							return res.send(retVal);
						}
					}
				});
